...
14. Dual write of pendientes: `python3 "$JBIN/repair-dualwrite.py" <MEMORY_DIR>` (dry-run). `rows_added=0 pipes_broken=0 shifted_rows=0 unrepairable=0 ids_invented=0 missing_data=0` is the expected result. Read `pipes_broken` (rows found), NOT `pipes_fixed` — in a dry-run nothing is repaired, so `pipes_fixed` is 0 by construction and reporting it would call a broken memory healthy. Anything else is a WARNING, and each count is a distinct silent loss of history:
