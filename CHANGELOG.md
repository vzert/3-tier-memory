...
## [2.12.0] - Unreleased
Pendiente antes de publicar: Fase 4 (correr `bin/test-journal-race.sh` en Linux y Git Bash; reintento de `os.replace` por antivirus) y el bump de `plugin.json`.
...
- **Pruebas** `bin/test-journal-race.sh` (2 workers x 15-16 eventos contra 2 compactadores simultaneos, 5/5 corridas limpias; un agente solo en < 1 s sin lock residual; lock huerfano robado exactamente una vez; replay byte a byte identico) y `bin/test-journal-guard.sh` (19 casos de la guardia; el arnes exige stderr vacio para no aceptar un traceback como "no bloquea").
