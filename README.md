...
- **Ten event types** — on action items, identified by a hash id shown as `_id: p-…_` at the end of the line: `pendiente.add`, `pendiente.resolve`, `pendiente.update` (corrects the text and/or priority of a LIVE item while keeping the id it was born with — the id is `sha1(text+creado+origen)`, so a text change would otherwise rename it and leave every citation of that id pointing at a closed item; the corrected line carries `_actualizado: FECHA_`, which is what tells `repair-dualwrite.py` the mismatch is deliberate), `pendiente.window`, `pendiente.expire` and `pendiente.reopen`. Plus `session.add`, `learning.add` (the rule number is assigned by the compactor, under the lock), `plan.upsert`, `research.upsert`.
