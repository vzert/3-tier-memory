...
- **Old `_pendientes.md` layouts** — the compactor anchors new pendientes under `## Alta/Media/Baja prioridad` (case-insensitive prefix). If your file predates 2.12.0 and lacks one of them, the SessionStart hook adds the missing headers on the next session (`bin/normalize-pendientes.py`, 2.12.1) without moving any item; `/audit-3t` reports it too.
- **Tests** — `bin/test-journal-race.sh` (2 workers against 2 concurrent compactors, 5/5 clean runs, byte-identical replay, orphaned lock stolen exactly once) and `bin/test-journal-guard.sh` (the guard, 19 cases; 23 on Git Bash). Run them from the plugin directory: `bash bin/test-journal-race.sh && bash bin/test-journal-guard.sh`. Both pass on macOS, Linux and Windows (Git Bash on `windows-latest`, 2.12.0), and on Windows a direct probe confirms that the lock's `mkdir` is atomic across processes.
